...
export default function ToolCard({
  ...
  return (
    <Link href={`/tools/${slug}`} className="group" onClick={handleClick}>
      <div className="bg-surface-elevated-dark rounded-2xl p-6 shadow-2xl card-hover h-full flex flex-col justify-between">
        <div>
          <h2 className="text-xl font-semibold text-white mb-2 group-hover:text-accent transition-colors">
            {title}
          </h2>
          <p className="text-text-secondary-dark text-sm">{desc}</p>
        </div>
